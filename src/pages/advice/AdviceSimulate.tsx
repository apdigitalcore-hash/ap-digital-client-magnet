  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
      <div className="relative h-16 w-16">
        <div className="absolute inset-0 rounded-full border-2 border-black/[0.1]" />
        <div className="absolute inset-0 animate-spin rounded-full border-2 border-transparent border-t-[#1d1d1f]" />
        <div className="absolute inset-3 rounded-full bg-black/[0.06] blur-md" />
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.p
          ...
          exit={{ opacity: 0, y: -6 }}
          className="mt-3 text-sm text-[#6e6e73]"
        >
          {LOADING_LINES[i]}
        </motion.p>
      </AnimatePresence>
      <div className="mt-8 h-1 w-64 overflow-hidden rounded-full bg-[#e8e8ed]">
        <motion.div
          className="h-full bg-[#1d1d1f] text-white"
          initial={{ width: '4%' }}
          animate={{ width: '92%' }}
        ...
        <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6 sm:py-16">
              ...
              <li key={s} className="flex flex-1 items-center gap-2">
                <span
                  className={`flex h-6 w-6 shrink-0 items-center justify-center rounded text-[11px] ${
                    i < step ? 'bg-[#1d1d1f] text-white' : i === step ? 'border border-[#1d1d1f] text-[#1d1d1f]' : 'border border-black/[0.12] text-[#86868b]'
                  }`}
                >
                  {i < step ? <Check className="h-3.5 w-3.5" /> : i + 1}
                </span>
                <span className={`hidden text-[11px] sm:inline ${i === step ? 'text-[#1d1d1f]' : 'text-[#86868b]'}`}>{s}</span>
                {i < STEPS.length - 1 && <span className="h-px flex-1 bg-[#e8e8ed]" />}
              </li>
            ))}
          </ol>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {step === 0 && (
                <>
                  <h1 className="text-3xl font-semibold tracking-[-0.03em] text-[#1d1d1f]">What are you advertising?</h1>
                  <div>
                  ...
                  <div>
                    ...
                    <select id="industry" className={field} value={v.industry} onChange={set('industry')}>
                      <option value="" className="bg-white">Choose an industry</option>
                  ...
                  <div>
                    ...
                    <p className="mt-1.5 text-xs text-[#86868b]">Age, location, interests and how ready they are to buy.</p>
                  </div>
                </>
              )}
              ...
              {step === 1 && (
                <>
                  <h1 className="text-3xl font-semibold tracking-[-0.03em] text-[#1d1d1f]">Where and how much?</h1>
                  <div>
                  ...
                  <div>
                    <div className="mb-3 flex items-baseline justify-between">
                      ...
                      <div className="flex items-center gap-1 text-sm">
                        ...
                        <input
                          ...
                          className="w-24 rounded-full border border-black/[0.1] bg-[#f5f5f7] px-2 py-1 text-right tabular-nums outline-none focus:border-[#1d1d1f]"
                          inputMode="numeric"
                          value={v.budget}
                    ...
                    />
                    <div className="mt-1 flex justify-between text-xs text-[#86868b]"><span>$500</span><span>$50,000+</span></div>
                  </div>
                </>
              )}
              ...
              {step === 2 && (
                <>
                  <h1 className="text-3xl font-semibold tracking-[-0.03em] text-[#1d1d1f]">Paste your ad</h1>
                  <div>
          ...
          {error && <p role="alert" className="mt-6 rounded-2xl border border-[#d70015]/20 bg-[#fff0f0] px-3.5 py-2.5 text-sm text-[#d70015]">{error}</p>}
