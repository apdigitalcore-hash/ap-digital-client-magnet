...
const PastelCTA = ({
  ...
  <section className="bg-[#EDEFF2] py-24 md:py-32">
    <div className="container-custom">
      <div className="mx-auto max-w-4xl overflow-hidden rounded-[2rem] bg-[linear-gradient(110deg,#f8e9c9_0%,#f3d9e6_28%,#e6dcf7_55%,#d7e6f7_78%,#dff1ea_100%)] px-6 py-14 text-center sm:px-12">
        ...
        <div className="mt-9 flex flex-col items-center justify-center gap-4 sm:flex-row">
          ...
          {showPhone && (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-foreground/25 px-7 py-3.5 text-xs font-semibold uppercase tracking-[0.14em] text-foreground transition-colors hover:bg-white/50"
            >
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-[11px] font-medium uppercase tracking-[0.18em] text-foreground/85">
          {['No long-term contracts', '90-day results guarantee', 'Free strategy session'].map((item, i) => (
            <span key={item} className="flex items-center gap-6">
              {i > 0 && <span className="hidden h-3 w-px bg-foreground/30 sm:block" />}
              {item}
            </span>
          ))}
        </div>
